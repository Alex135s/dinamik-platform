function StatsRow({ project, docsCount, tasks }) {
  ...
  if (project.status === 'completado') {
    diasValue = '✓'; diasLabel = 'Completado'
  } else if (endD) {
  ...
  const stats = [
    { label: 'Progreso',    value: `${project.progress || 0}%`,      icon: '📊', bg: 'bg-orange-100 text-orange-600' },
    { label: 'Entregables', value: docsCount,                        icon: '📁', bg: 'bg-blue-100 text-blue-600'   },
    { label: 'Tareas',      value: `${tareasComp}/${tasks.length}`,  icon: '✅', bg: 'bg-green-100 text-green-600' },
    { label: diasLabel,     value: diasValue,                        icon: '⏱️', bg: 'bg-amber-100 text-amber-600' },
  ]
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {stats.map((s, i) => (
        <div key={i} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-base mb-2 ${s.bg}`}>
            {s.icon}
          </div>
